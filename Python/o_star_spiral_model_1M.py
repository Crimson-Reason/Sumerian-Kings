"""
O-star spiral-arm model with 1M samples for high-precision statistics.
Large-sample run with N_mc = 1,000,000 for tighter Monte Carlo error bars.
"""
...
# Population
N_total = 30000.0  # nominal total O stars in Milky Way
...
sigma0 = compute_sigma0(N_total, Rd, Rmax)
...
# ===== Arm membership (local phi grid search - VECTORIZED) =====
print(f"Evaluating arm membership...")
phi_window_half = math.radians(6.0)
n_phi_samples = 61  # reduced for speed (was 121)
...
for arm in arm_params:
    ...
    # Vectorized: create grid all at once to avoid nested loop
    phi_grid = phi_gc[:, np.newaxis] + phi_offsets[np.newaxis, :]
    phi_rel = (phi_grid - phi_ref + math.pi) % (2.0*math.pi) - math.pi
    R_arm_grid = R_ref * np.exp(phi_rel * tan_p)
    radial_sep_grid = np.abs(R_gc[:, np.newaxis] - R_arm_grid)
...
for N_tot in [20000, 30000, 50000]:
    ...
    print(f"N_total={N_tot:5d} -> Expected in arms ≈ {N_loc:.2f}")
...
output_file = 'o_star_results_1M.json'
